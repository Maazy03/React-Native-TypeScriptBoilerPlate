...
import {FlatListPropsType} from '@components/constants/types';

const wait = (timeout: any) => {
  return new Promise((resolve: any) => setTimeout(resolve, timeout));
};

const FlatList = (props: FlatListPropsType) => {
  const {
    containerStyles,
  ...
  const [pageNo, setPageNo] = useState<number | undefined>(pageNumber);
  ...
  useEffect(() => {
    if (!isDeleted) {
      if (!pageNo || pageNo === 1) {
        ...
        setData((prevState: any) => [...prevState, ...listData]);
      }
      setFooterLoader(false);
    } else {
      let updatedArray = data?.filter((item: any) => item?._id !== deleteId);
    ...
    return (
      footerLoader && (
        <View style={styles.footerContainer}>
          <Loader iosLoader={true} size={30} color={R.color.primaryColor1} />
        </View>
      )
    );
  };
